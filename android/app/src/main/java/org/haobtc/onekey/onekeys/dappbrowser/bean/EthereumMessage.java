...
/** Class for EthereumMessages to be signed. Weiwu, Aug 2020 */
public class EthereumMessage implements Signable {
    ...
    public EthereumMessage(
        ...
        this.messageBytes = HexUtils.hexStringToByteArray(message);
    ...
    public CharSequence getUserMessage() {
        if (messageType != SignMessageType.SIGN_PERSONAL_MESSAGE
                || !StandardCharsets.UTF_8.newEncoder().canEncode(userMessage)) {
            return userMessage;
        } else {
    ...
    private String hexToUtf8(CharSequence hexData) {
        String hex = HexUtils.cleanHexPrefix(hexData.toString());
