...
public class BusinessAsyncTask extends AsyncTask<String, Void, String> {
  ...
  protected String doInBackground(String... strings) {
    ...
    switch (strings[0]) {
      ...
      case SIGN_MESSAGE:
        try {
          result =
              Daemon.commands.callAttr(strings[0], strings[1], strings[2], strings[3]).toString();
        } catch (Exception e) {
          cancel(true);
          onException(e);
        }
      ...
      case IMPORT_MNEMONIC:
        try {
          result =
              Daemon.commands
                  .callAttr(strings[0], strings[1], strings[2], strings[3], strings[4])
                  .toString();
        } catch (Exception e) {
          cancel(true);
          onException(e);
      ...
      case APPLY_SETTING:
        try {
          if ("setBluetooth".equals(strings[2])) {
            ...
            result =
                Daemon.commands
                    .callAttr(strings[0], strings[1], new Kwarg("label", strings[3]))
                    .toString();
            ...
            result =
                Daemon.commands
                    .callAttr(
                        strings[0],
                        strings[1],
                        new Kwarg("auto_lock_delay_ms", shutdownTime * 1000))
                    .toString();
          } else if ("fix_hardware_language".equals(strings[2])) {
            result =
                Daemon.commands
                    .callAttr(strings[0], strings[1], new Kwarg("language", strings[3]))
                    .toString();
          }
        } catch (Exception e) {
          cancel(true);
          ...
          result =
              Daemon.commands
                  .callAttr(
                      strings[0],
                      strings[1],
                      new Kwarg("type", WhiteListEnum.Inquire.getWhiteListType()))
                  .toString();
        } catch (Exception e) {
          onException(e);
        }
            ...
            result =
                Daemon.commands
                    .callAttr(
                        strings[0],
                        strings[1],
                        new Kwarg("type", WhiteListEnum.Add.getWhiteListType()),
            ...
            result =
                Daemon.commands
                    .callAttr(
                        strings[0],
                        strings[1],
                        new Kwarg("type", WhiteListEnum.Delete.getWhiteListType()),
                        new Kwarg("addr_in", strings[3]))
                    .toString();
          }
        } catch (Exception e) {
  ...
  private void onException(Exception e) {
    if (Objects.nonNull(e.getMessage())
            && HardWareExceptions.PASSPHRASE_OPERATION_TIMEOUT.getMessage().equals(e.getMessage())
