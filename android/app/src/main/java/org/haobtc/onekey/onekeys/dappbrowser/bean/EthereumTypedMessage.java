...
import org.haobtc.onekey.onekeys.dappbrowser.utils.MessageUtils;
...
public class EthereumTypedMessage implements Signable {
    ...
    public EthereumTypedMessage(
            byte[] value, CharSequence userMessage, String displayOrigin, long leafPosition) {
    ...
    public EthereumTypedMessage(
            ...
            String domainName,
            long callbackId,
            CryptoFunctionsInterface cryptoFunctions) {
        try {
            try {
                ProviderTypedData[] rawData =
                        new Gson().fromJson(messageData, ProviderTypedData[].class);
    ...
    // User message is the text shown in the popup window - note CharSequence is used because
    // message contains text formatting
