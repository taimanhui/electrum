...
public class ReceivedPageActivity extends BaseActivity {
    ...
    @OnClick({
        ...
        R.id.button,
        R.id.img_back,
        R.id.text_check_address,
        R.id.text_change_address,
        R.id.textView5
    })
        ...
        switch (view.getId()) {
                                    ...
                                    if (granted) { // Always true pre-M
                                        ...
                                        if (uri != null) {
                                            ...
                                            shareIntent.putExtra(
                                                    Intent.EXTRA_STREAM, Uri.parse(uri));
            ...
            case R.id.textView5:
                ...
                Objects.requireNonNull(cm2, "ClipboardManager not available")
                        .setPrimaryClip(ClipData.newPlainText(null, textView5.getText()));
                Toast.makeText(ReceivedPageActivity.this, R.string.copysuccess, Toast.LENGTH_LONG)
                        .show();
                break;
            default:
        }
            ...
            if (next == 0) {
                ...
                walletAddressShowUi =
                        PyEnv.sCommands.callAttr(
                                "get_wallet_address_show_UI", new Kwarg("next", next));
            }
    ...
    private String saveImageToGallery(Context context, @NonNull Bitmap bmp) {
        // Save picture
        String storePath =
                context.getExternalFilesDir(Environment.DIRECTORY_PICTURES) + File.separator;
        ...
        try {
            ...
            String uri =
                    MediaStore.Images.Media.insertImage(
                            context.getContentResolver(), file.getAbsolutePath(), fileName, null);
            // Send broadcast notice to update database after saving picture
            context.sendBroadcast(
                    new Intent(Intent.ACTION_MEDIA_SCANNER_SCAN_FILE, Uri.parse(uri)));
            return uri;
        } catch (IOException e) {
            e.printStackTrace();
        } finally {
