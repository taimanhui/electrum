...
public class HardwareDetailsActivity extends BaseActivity implements BusinessAsyncTask.Helper {
  ...
  private void getXpub() {
    new BusinessAsyncTask()
        .setHelper(this)
        .execute(
            BusinessAsyncTask.GET_EXTEND_PUBLIC_KEY, MyApplication.getInstance().getDeviceWay());
  }
  ...
  private void verification(String result) {
    ...
    OkHttpUtils.post()
        .url("https://key.bixin.com/lengqian.bo/")
        .params(pramas)
        .build()
        .connTimeOut(10000)
        .execute(
            new StringCallback() {
              ...
              public void onResponse(String response) {
                try {
                  HardwareVerifyResponse verifyResponse =
                      HardwareVerifyResponse.objectFromData(response);
                  if (verifyResponse.isIsBixinkey() && verifyResponse.isIsVerified()) {
                    ...
                    String str =
                        PreferencesManager.get(
                                HardwareDetailsActivity.this, Constant.DEVICES, deviceId, "")
                            .toString();
  ...
  public void onException(Exception e) {
    switch (currentMethod) {
      ...
      case BusinessAsyncTask.COUNTER_VERIFICATION:
        EventBus.getDefault()
            .post(new VerifyFailedEvent(VerifyFailedEvent.FailedReason.GOT_CERT_FAILED));
        break;
    }
  }
