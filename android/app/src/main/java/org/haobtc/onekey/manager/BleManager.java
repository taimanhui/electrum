...
public final class BleManager {
                ...
                permission -> {
                  if (permission.granted) {
                    ...
                    new XPopup.Builder(fragmentActivity)
                        ...
                        .asCustom(new RequestLocationPermissionsDialog(fragmentActivity))
                        .show();
                  }
                });
  }
  ...
  private final BleConnectCallback<BleDevice> mConnectCallback =
      new BleConnectCallback<BleDevice>() {
        ...
        public void onConnectionChanged(BleDevice device) {
          if (device.isDisconnected() && Objects.equals(currentAddress, device.getBleAddress())) {
            connecting = false;
          }
        }
  ...
  private void dealBleResponse(BluetoothGattCharacteristic characteristic, StringBuffer buffer) {
    ...
    if ((Integer.parseInt(
                buffer.substring(
                    Constant.LENGTH_FILED_START_OFFSET, Constant.LENGTH_FILED_END_OFFSET),
                16)
  ...
  private boolean checkGpsEnable() {
    if (locationManager == null) {
      locationManager =
          (LocationManager)
              Objects.requireNonNull(fragmentActivity.getSystemService(LOCATION_SERVICE));
    }
