...
public class BleService extends Service {
    ...
    private final IntentFilter bondFilter =
    ...
    private final BleWriteCallback<BleDevice> writeCallBack =
            new BleWriteCallback<BleDevice>() {
                ...
                public void onWriteSuccess(
                    ...
                    // CommonUtils.bytes2hex(characteristic.getValue()));
        ...
        mBle.enableNotify(
                ...
                new BleNotiftCallback<BleDevice>() {
                    ...
                    public void onChanged(
                        ...
                        if ((Integer.parseInt(buffer.substring(10, 18), 16) + 9)
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        bluetoothDevice =
                BluetoothAdapter.getDefaultAdapter().getRemoteDevice(mBleDevice.getBleAddress());
        // 禁止同时连接多个BiXinKEY设备
        mBle.getConnetedDevices().stream()
                .filter(bleDevice -> !bleDevice.getBleAddress().equals(mBleDevice.getBleAddress()))
        ...
        if (isDfu) {
            ...
            new Handler()
                    .postDelayed(
                            () ->
                                    EventBus.getDefault()
                                            .postSticky(new DfuEvent(DfuEvent.START_DFU)),
                            2000);
        } else {
            isBonded = false;
            switch (bluetoothDevice.getBondState()) {
                ...
                case BluetoothDevice.BOND_NONE:
                    ...
                    if (!bond) {
                        EventBus.getDefault().post(new ExitEvent());
                        Log.e("BLE", "无法绑定设备");
                        Toast.makeText(this, getString(R.string.dont_band), Toast.LENGTH_SHORT)
                                .show();
                    }
                    isBonded = true;
                    break;
                        ...
                        if (BluetoothDevice.ACTION_BOND_STATE_CHANGED.equals(action)) {
                            ...
                            if (state == BluetoothDevice.BOND_BONDED
                                ...
                                mHandler.postDelayed(
                                        () -> EventBus.getDefault().postSticky(new DfuEvent(3)),
                                        2000);
                                return;
                            }
                            if (state == BluetoothDevice.BOND_NONE
                                ...
                                Log.d(
                                        TAG,
                                        String.format(
                                                "设备==%s==,配对信息已清除", bluetoothDevice.getName()));
                            }
                            ...
                            if (state == BluetoothDevice.BOND_NONE
                                ...
                                Log.d(
                                        TAG,
                                        String.format(
                                                "您已拒绝与设备==%s==配对", bluetoothDevice.getName()));
                                return;
                            }
                            if (state == BluetoothDevice.BOND_BONDED
                                    && previousState == BluetoothDevice.BOND_BONDING
