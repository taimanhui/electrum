    ...
    public void onViewClicked(View view) {
        switch (view.getId()) {
            ...
            case R.id.local:
                ...
                if (isNFC) {
                    Intent intent =
                            new Intent(this, CommunicationModeSelector.class).setAction("backup");
                    intent.putExtra(
                            "get_feature", getIntent().getBooleanExtra("get_feature", false));
                    startActivity(intent);
                } else {
                    if (Ble.getInstance().getConnetedDevices().size() != 0) {
                        new BusinessAsyncTask()
                                .setHelper(this)
                                .execute(BusinessAsyncTask.BACK_UP, COMMUNICATION_MODE_BLE);
                    } else {
