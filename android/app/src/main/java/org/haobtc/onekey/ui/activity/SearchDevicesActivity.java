...
public class SearchDevicesActivity extends BaseActivity
  ...
  public void init() {
    mSearchMode =
        getIntent()
            .getIntExtra(
                Constant.SEARCH_DEVICE_MODE, Constant.SearchDeviceMode.MODE_PAIR_WALLET_TO_COLD);
    addBleView();
  ...
  public void onBleScanStop(BleScanStopEvent event) {
    ...
    if (mLayoutView.getHeight() > 0) {
      mLayoutView.post(
          () -> ValueAnimatorUtil.animatorHeightLayout(mLayoutView, mLayoutView.getHeight(), 0));
    }
  }

  public void addBleView() {
    mLayoutView =
        (LinearLayout) LayoutInflater.from(this).inflate(R.layout.layout_search_device_ble, null);
            ...
            case Constant.SearchDeviceMode.MODE_RECOVERY_WALLET_BY_COLD:
              if (features.isInitialized()) {
                if (features.isBackupOnly()) {
                  boolean hasLocalHd =
                      (boolean)
                          PreferencesManager.get(this, "Preferences", Constant.HAS_LOCAL_HD, false);
                  if (hasLocalHd) {
  ...
  private void update(HardwareFeatures features) {
    ...
    String bleMac =
        PreferencesManager.get(this, org.haobtc.onekey.constant.Constant.BLE_INFO, bleName, "")
            .toString();
  ...
  private Bundle getBundle(
      String urlPrefix, String locate, String info, String curNrfVersion, boolean isBootloader) {
