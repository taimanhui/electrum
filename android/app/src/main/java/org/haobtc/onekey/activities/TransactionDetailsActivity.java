...
public class TransactionDetailsActivity extends BaseActivity {
    ...
    public void initView() {
        ...
        if (!TextUtils.isEmpty(intent.getStringExtra("signed_raw_tx"))) {
            ...
            EventBus.getDefault()
                    .post(new CheckReceiveAddress("finish_mode_select")); // close sign page
        } else {
            ...
            unConfirmStatus =
                    intent.getStringExtra(
                            "unConfirmStatus"); // Manually change to the status to be confirmed
    ...
    private void jsonDetailData(String detail) {
        ...
        if (size != 1) {
            if ("English".equals(language)) {
                ...
                tetAddressNum.setText(
                        String.format(
                                "%s%d%s", getString(R.string.wait), size, getString(R.string.ge)));
            }
        } else {
        ...
        if (inputAddr.size() != 1) {
            if ("English".equals(language)) {
                tetPayAddressNum.setText(
                        String.format("%s%d", getString(R.string.wait), inputAddr.size()));
            } else {
                tetPayAddressNum.setText(
                        String.format(
                                "%s%d%s",
    ...
    private void scanDataDetailMessage() {
        Gson gson = new Gson();
        ScanCheckDetailBean scanCheckDetailBean =
                gson.fromJson(strParse, ScanCheckDetailBean.class);
    ...
    private void judgeState(String txStatus) {
        if ("broadcast_complete".equals(unConfirmStatus)) {
            if (!braodStatus) {
                broadcastStatus(); // sendOne2OnePageActivity  1-n wallet -->sign and broadcast
            ...
            if ("Unconfirmed".equals(txStatus)) { // Unconfirmed
                ...
                if (mathMax > 0) {
                    tetConfirm.setText(
                            String.format("%s%s", getString(R.string.confirmnum), ">100"));
                } else {
                    tetConfirm.setText(
                            String.format("%s%s", getString(R.string.confirmnum), strConfirl));
                }
            ...
            } else if (txStatus.contains("Signed")
                    || txStatus.contains("Local")
                    || canBroadcast) { // signed
    ...
    public void onViewClicked(View view) {
        switch (view.getId()) {
            ...
            case R.id.btn_share:
                Intent intent =
                        new Intent(TransactionDetailsActivity.this, ShareOtherActivity.class);
            ...
            case R.id.lin_getMoreaddress:
                ...
                Intent intent1 =
                        new Intent(
                                TransactionDetailsActivity.this, DeatilMoreAddressActivity.class);
    ...
    private void ifHaveRbf() {
        ...
        if (getRbfFeeInfo != null) {
            ...
            try {
                ...
                viewSpeed
                        .findViewById(R.id.btn_add_Speed)
                        .setOnClickListener(
                                v -> {
                                    confirmedSpeed();
                                });
    ...
    private void receiveAddSpeed() {
        ...
        if (getRbfFeeInfo != null) {
            ...
            View viewSpeed =
                    LayoutInflater.from(this).inflate(R.layout.receive_add_speed, null, false);
            alertDialog = new AlertDialog.Builder(this).setView(viewSpeed).create();
            Objects.requireNonNull(alertDialog.getWindow())
                    .setBackgroundDrawable(new ColorDrawable(Color.TRANSPARENT));
            ImageView imgCancel = viewSpeed.findViewById(R.id.cancel_select_wallet);
            ...
            try {
                ...
                String recommendFeeRate =
                        feeRateForChild.substring(0, feeRateForChild.indexOf(" "));
                ...
                if (recommendFeeRate.contains(".")) {
                    String recommendIntFee =
                            recommendFeeRate.substring(0, recommendFeeRate.indexOf("."));
                ...
                tetNewfee.setText(
                        String.format(
                                "%s  %s", getString(R.string.speed_fee_is), feeForChild + " sat"));
    ...
    private void seekBarReceiveFee(
            IndicatorSeekBar seekBar,
            TextView testChangeFee,
            TextView tetNewfee,
            int recommendProRate) {
        seekBar.setOnSeekBarChangeListener(
                new IndicatorSeekBar.OnIndicatorSeekBarChangeListener() {
                    ...
                    public void onStopTrackingTouch(SeekBar seekBar) {
                        ...
                        BigDecimal bigChange =
                                new BigDecimal(seekBar.getProgress() + (minProRate * 10000));
        ...
        try {
            getRbfFeeInfo =
                    PyEnv.sCommands.callAttr(
                            ...
                            new Kwarg(
                                    "suggested_feerate",
                                    Float.parseFloat(String.valueOf(newFeerate))));
        } catch (Exception e) {
            e.printStackTrace();
            if (e.getMessage().contains("max fee exceeded")) {
                mToast(getString(R.string.fee_exceeded));
    ...
    private void seekbarLatoutup(
            IndicatorSeekBar seekBar, TextView testChangeFee, TextView tetNewfee) {
        seekBar.setOnSeekBarChangeListener(
                new IndicatorSeekBar.OnIndicatorSeekBarChangeListener() {
                    @Override
                    public void onProgressChanged(
                    ...
                    public void onStopTrackingTouch(SeekBar seekBar) {
                        ...
                        BigDecimal bigChangeRbf =
                                new BigDecimal(seekBar.getProgress() + (minPro * 10000));
    ...
    private void createBumpFee(TextView tetNewfee, BigDecimal newFee) {
        ...
        if (createBumpFee != null) {
            ...
            Gson gson = new Gson();
            AddspeedNewtrsactionBean addspeedNewtrsactionBean =
                    gson.fromJson(strContent, AddspeedNewtrsactionBean.class);
    ...
    private void confirmedReceiveSpeed(String feeForChild) {
        ...
        if (createCpfpTx != null) {
            ...
            Gson gson = new Gson();
            AddspeedNewtrsactionBean addspeedNewtrsactionBean =
                    gson.fromJson(strNewTx, AddspeedNewtrsactionBean.class);
    ...
    private void signInputpassDialog() {
        View view1 =
                LayoutInflater.from(TransactionDetailsActivity.this)
                        .inflate(R.layout.input_wallet_pass, null, false);
        AlertDialog alertDialog =
                new AlertDialog.Builder(TransactionDetailsActivity.this).setView(view1).create();
        alertDialog.getWindow().setBackgroundDrawable(new ColorDrawable(Color.TRANSPARENT));
        EditText strPass = view1.findViewById(R.id.edit_password);
        view1.findViewById(R.id.btn_enter_wallet)
                .setOnClickListener(
                        v -> {
        ...
        view1.findViewById(R.id.cancel_select_wallet)
                .setOnClickListener(
                        v -> {
                            alertDialog.dismiss();
                        });
    ...
    private void confirmedSpeed() {
        ...
        if (createBumpFee != null) {
            ...
            tetAddSpeed.setVisibility(View.GONE);
            buttonStatus =
                    false; // buttonStatus = true-->Click the button   ：  false-->Modify status only
            edit.putString("signedRowTransaction", publicTrsation);
            edit.apply();
            mCreataSuccsesCheck();
            alertDialog.dismiss();
