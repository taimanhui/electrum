...
from electrum_gui.common.basic.orm.migrate.manager import migrating
from electrum_gui.common.basic.orm.migrate.models import MigrationRecord


@test_utils.cls_test_database(MigrationRecord)
class TestMigrate(TestCase):
    @patch("electrum_gui.common.basic.orm.migrate.manager.SqliteMigrator")
    @patch("electrum_gui.common.basic.orm.migrate.manager.pkgutil")
    @patch("electrum_gui.common.basic.orm.migrate.manager.importlib")
    @patch("electrum_gui.common.basic.orm.migrate.manager.settings")
    def test_migrating(self, fake_settings, fake_importlib, fake_pkgutil, fake_sqlite_migrator_creator):
        ...
        fake_migration_a_dir = Mock(
            __file__=True, __path__="/repo/src/module_a/migrations", __name__="src.module_a.migrations"
        )
        fake_migration_a_01 = Mock(
            __name__="src.module_a.migrations.01_init",
            update=Mock(side_effect=partial(fake_update_callback, test_tag="a_01")),
        )
        fake_migration_a_04 = Mock(
            __name__="src.module_a.migrations.04_update",
            update=Mock(side_effect=partial(fake_update_callback, test_tag="a_04")),
        )

        fake_migration_b_dir = Mock(
            __file__=True, __path__="/repo/src/module_b/migrations", __name__="src.module_b.migrations"
        )
        fake_migration_b_01 = Mock(
            __name__="src.module_b.migrations.01_init_models",
            update=Mock(side_effect=partial(fake_update_callback, test_tag="b_01")),
        )
        fake_migration_b_02 = Mock(
            __name__="src.module_b.migrations.02_update_models",
